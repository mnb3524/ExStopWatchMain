...
public final class ExStopWatch extends JavaPlugin {
    ...
    public void showAllStopwatch(CommandSender s) {
        curPlayer = null;
        s.sendMessage(ChatColor.GREEN + "タイマーが全プレイヤーに見えています！");
    }
    ...
    public void showStopwatch(CommandSender s, String[] args) {
        ...
        if (player == null) {
            s.sendMessage(ChatColor.RED + "不明なプレイヤーのエラーです。");
            return;
        }
    ...
    public void pauseStopwatch(CommandSender s) {
        if (!isRunning) {
            s.sendMessage(ChatColor.RED + "タイマーが起動していません！");
        } else {
            if (isPaused) {
                stopWatch.resume();
    ...
    public void getItems(CommandSender s) {
        if (!(s instanceof Player)) {
            s.sendMessage(ChatColor.RED + "タイマー用アイテムはプレイヤーしか使えません");
            return;
        }
        Player p = (Player) s;
    ...
    public void offsetSeconds (CommandSender s, String[] args) {
        ...
        try {
            ...
            s.sendMessage(ChatColor.GREEN + "オフセットを " + offsetArg +" で設定。 現在のオフセットは " + offsetSec);
